...
namespace BuildDataFromDb.Services
{
    ...
    public class UserService : ICRUD<UserIdentityModel>
    {
        ...
        public async Task<bool> UpdateAsync(UserIdentityModel id) {//Actualizar la contraseña de usuario
            bool flag = false;
            //Obtenemos el objeto de la tabla a actualiza
            var objusuario = await _accesoFircoContext.Usuarios.FirstOrDefaultAsync(e => e.Usuario1 == id.Usuario);
            ...
            try
            {
                if (objusuario != null)
                {
                    ...
                    await _accesoFircoContext.SaveChangesAsync();   //Actualizacion de contraseña temporal 
                    flag = true;    
                }
        ...
        {          
           ...
           var empleadoDb = await _dbempleadosContext.ViewEmpleados.Where(item => item.NoEmpleado == id)
            ...
            foreach (var item in usuarioDb.AsQueryable())
            {
                ...
                //Bandera de si ya fue reseteado este usuario o es la primera vez que ingresa
                objetoUsuario.Resetado = item.PassCtrl;
            }
