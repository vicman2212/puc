...
namespace Message.Mail
{
    public class SendMailService:IMessageBuilder<MessageModel>
    {
        ...
        public SendMailService() {
            ...
            _DefaultMail = build["MailConfiguration:DefaultMail"];
        }
        ...
        public async Task<bool> SendAsync(MessageModel message)
        {
            ...
            try
            {
                ...
                //Procedimiento cuando mandamos la lista de copias vacia
                if (message.CCP == null)
                {
                    //Agregamos el primer elemento que es el mail por default
                    message.CCP = new List<string> { _DefaultMail };
                }
                else {//Cuando traemos una lista correos con copia, al final siempre agregamos la cuenta por default
                    message.CCP.Add(_DefaultMail);
                }
