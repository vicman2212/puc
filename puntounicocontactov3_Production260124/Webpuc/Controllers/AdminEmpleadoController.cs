...
namespace Webpuc.Controllers
{
    ...
    public class AdminEmpleadoController : Controller
    {
        ...
        //insertar elementos
        [HttpPost]
        public async Task<bool> InsertEmpleado(InsertEmpModel empleado) {

            //2.- Procedimiento para determinar un correo 

            GenerarCorreoService generarCorreoService = new GenerarCorreoService(_empleados);
            string correo = await generarCorreoService.GenerateEMail(empleado.Nombres, empleado.ApellidoPaterno, empleado.ApellidoMaterno);
            ...
            if (await empleadoDb.InsertAsync(empleadoDbobject))
            {
                ...
                MessageModel message = new MessageModel()
                {
                    ...
                    CCP = new List<string> { "soporte.tic@firco.gob.mx" },
                    Subject = "!ATENCION!, Actualizacion de Empleado",
                    BodyMessage = messageNotifcService.MsnInsertEmpleado(empleadoDbobject)
                };
                await messageBuilder.SendAsync(message);
        ...
        //**************************************************************************************************
        //********************************SUBRUTINAS DE VALIDACION******************************************
        public async Task<bool> ExistNoEmpleado(Empleado emp) {

            if (await empleadoDb.ExistAsync(emp) == 0)//Si no se encontraron elementos
            {
                return false;
            }
            return true;        
